...
export default function Deposit() {
    ...
    useEffect(() => {
        async function fetchdata() {
            await axios.get('https://server-1-06n9.onrender.com/data').then((item) => {
                setData(item.data);
                console.log(data);
...
// console.log(name);
        if (isNaN(deposit) || deposit < 0) {
            ...
            await axios.put(`https://server-1-06n9.onrender.com/${id}`, { amount: total + deposit }).then(setTotal(total + deposit)).then(alert(`amount ${deposit} is deposited successfully !`));
...
//             await axios.put(`https://server-1-06n9.onrender.com//update/${dataId}`,{amount:total+deposit}).then(setTotal(total + deposit)).then(alert(`amount ${deposit} is deposited successfully !`));
...
//                         {/* <h1 id='Dep'>Deposit</h1> */}
//                         {/* <Form.Group className="mb-3">
//                 <Form.Control type="text" placeholder="Enter userName" onChange={(e)=>changeHandler(e.target.value,setName)} />
//             </Form.Group>
//             <Form.Group className="mb-3">
//                 <Form.Control type="password" placeholder="Enter password" onChange={(e)=>changeHandler(e.target.value,setPass)}/>
//             </Form.Group> */}
//                         <Form.Group className="mb-3">
//                             <Form.Control type="number" placeholder="Enter amount" onChange={(e) => changeHandler(Number(e.target.value), setDeposit)} />
//                         </Form.Group>
